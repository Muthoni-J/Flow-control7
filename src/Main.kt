fun main(){
    oddnumbers()
    var names = peoplesnames(arrayOf("Gumato", "Moureen", "Charity", "Joan"))
        println(names)
    guestdrinks(8)
    multiplenums()
}
...
//names longer than 5 characters (2pt
fun peoplesnames(name:Array<String>):Int{
    var y=0
    for (n in name){
        if (n.length>5){
            y++
...
//printing out the name of the drink each person gets according to their age.2pts

fun guestdrinks(age:Int) {
    if (age<6) {
...
//5, print "FizzBuzz" instead of the number. (4p
fun multiplenums() {
    for (num in 1..100) {
        if (num % 3 == 0 && num % 5 == 0){
            println("FizzBuzz")
